...
def getInformationDinero(page):
    """Get information page Dinero titulo,medio,fecha,hora."""
    data = []
    soup = BeautifulSoup(page.text, 'html.parser')
    articles =  soup.findAll("article",{"class":"article short"})
    for article in articles:
        title = article.findAll("h2",{"class":"article-h"})
        title = title[0].text
        date =  datetime.today()
        date =  date.strftime("%d/%m/%Y")
        hour = '' 
        data+=[[str(title),'Dinero',date,hour]]
    return data
...
pageRequests('https://www.dinero.com/','Dinero',getInformationDinero)
